<template>
  <div class="course_block_container" v-for="(group, g_index) in courseData" :key="g_index">
    ...
    <div class="group_field">
      <div class="course_block" v-for="(item, index) in group.courses" :key="index" @click="$nf.not_finish()">
        ...
        <div class="course_info_container">
          <div class="info_f">
            <span class="ci_title">{{ item.title }}</span>
            <span class="ci_other"><span class="ci_other-name">课程编号：</span><span
                class="ci_other-value">{{ item.id }}</span></span>
            <span class="ci_other"><span
                class="ci_other-name">主讲教师：</span><span class="ci_other-value">{{ item.teacher }}</span></span>
          </div>
          <div class="source_f">
            来自<span>{{ item.source }}</span>
          </div>
        </div>
      </div>
    </div>
...
.course_info_container {
  background-image: linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, var(--theme-color) 30.73%);
  padding: 21px 12px;
  padding-bottom: 21px;
  color: white;
